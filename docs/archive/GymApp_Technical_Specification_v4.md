...
// IMPORTANT: Verify these IDs against live WGER API before using
module.exports = {
  1: 'barbell', 3: 'dumbbell', 4: 'machine', 5: 'machine',
  6: 'bodyweight', 7: 'resistance_band', 8: 'kettlebell',
  // Add any missing IDs as 'other'
};
```
...
4. Upload directly to Supabase Storage from mobile:
   ...
   await supabase.storage.from('workout-media').upload(path, compressedFile);
   ```
...
8. `cd backend && node scripts/import-wger.js` — verify WGER equipment IDs against live API first; expect > 100 exercises
...
*Resolves all 80 gaps identified in v3 review.*
