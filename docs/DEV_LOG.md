...
Founder said the post-PR1 admin redesign felt flat: "solid colour, no photos, no fluid animations, want it to feel alive, every click should feel like a journey." Spawned a fresh 4-agent council (motion engineer, art director, interaction designer, creative technologist) to critique the prior plan and propose a richer scope. All four converged: scrap PR2/3/4+ as-planned; redraw around motion-as-system + photography-as-system + two signature artifacts (Ember Seam + Live Pulse Strip).

Per founder request, executed via **8 parallel worktree agents organized into 2 teams** rather than the inline default. To avoid the multi-agent stall pattern from Phase A (DEV_LOG documents it twice), used a strict file-ownership matrix with zero overlap, plus pre-staged stubs in master so beta worktrees could compile without waiting on alpha.

**Pre-stage (commits `ec6793b` + `480cab9`):**
- Installed `framer-motion@12` + `@number-flow/react` in admin (~21 kB gz + ~6 kB gz with LazyMotion).
- Created stub files for `LivePulseStrip`, `EmberSeam`, `PrimaryButton`, `lib/motion.ts` so both teams' worktrees forked from a state where every cross-team import already resolves. Alpha-4's job was to replace the stubs with real implementations; beta agents imported from the stable API paths regardless.

**Team Alpha — motion foundation (4 worktree agents in parallel):**
- **α1** `worktree-agent-a8b029238de097b86` (commit `40d18c6`) — wrapped `<Routes>` in `<AnimatePresence mode="wait" initial={false}>` keyed on `useLocation().pathname`. Each route's element wrapped in a `<PageShell>` with asymmetric in/out timings (180ms exit, 240ms entry with 60ms delay) and Vercel ease `[0.32, 0.72, 0, 1]`. ProtectedRoute still wraps gated pages with Layout; PageShell sits inside so the chrome doesn't re-animate on every nav.
- **α2** `worktree-agent-a84cf0f37bbdf9ea4` (commit `6def0fa`) — replaced the 2px static active-bar with a `<motion.span layoutId="navActivePill">` inside a `<LayoutGroup>` so framer slides the same DOM node between nav items on `springModal` (380/32). Active state choreography: Lucide `strokeWidth` 1.75→2.25, label tracking-normal→tracking-tight, both with `duration-200` synchronized settle. Mounts `<LivePulseStrip />` at the very top of the shell (z-50, fixed, edge-to-edge) and `<EmberSeam />` along the topbar bottom edge.
- **α3** `worktree-agent-aeb8af70a19ef7992` (commit `0e2723d`) — additive `@layer components` block in `index.css` adding `.ember-seam` (1px hairline + radial-style ember gradient + 8s breathing background-position + 12px drop-shadow halo), `.hover-lift` (translateY -2px + shadow + border tighten on iOS spring-out 160ms), `.press-compress` (scale 0.97 :active 60ms), `.link-underline-draw` (left-to-right background-size animation 240ms), `.pulse-travel` (12% ember sliver sweeping in 1.2s for LivePulseStrip), plus `prefers-reduced-motion` guard. Tailwind config got 3 new keyframes (`ember-breathe`, `pulse-travel`, `ken-burns`) + the `animate-ken-burns` utility. Caught and self-corrected the worktree absolute-path leak that several agents tripped over.
- **α4** `worktree-agent-a8b4769ec843b7954` (commit `8000259`) — replaced all four stubs with real implementations. `motion.ts` keeps the original public API (VERCEL_EASE, pageTransition, listStagger, listItem, springModal) plus new `springMagnetic` and `hoverLift` exports. `EmberSeam` defers to the CSS class with optional `vertical` rotation. `LivePulseStrip` uses `useReducedMotion`, random 6-12s pulse interval, 40%-of-the-time counter drift in [0, 12], proper cleanup. `PrimaryButton` upgraded to `motion.button` with `magnetic` prop (cursor-proximity tug at 0.18 strength via `useMotionValue` + `useSpring`, auto-disabled under reduced motion), inset ember box-shadow on hover, Loader2 spinner for `loading={true}`, chained user mouse handlers.

**Team Beta — signature pages (4 worktree agents in parallel):**
- **β1** `worktree-agent-a8c951b1d66b41464` (commit `d6ee08a`) — Dashboard "First Light" rebuild. EmberSeam under page header, then a 280px hero band with Anastase Maragos's chalk-hands macro from Unsplash (`photo-1517836357463-d25dfeac3438`), B&W via `grayscale(100%) contrast(1.1) brightness(0.9)`, left-anchored 40%→transparent mask so KPIs sit in clean negative space, 8% ember bottom-up multiply-blend. Hero hosts headline + the live "3 lifting now" pulse (ember dot + breathing ring). Below: 4 stat cards using `<NumberFlow value={n} />` for digit-by-digit slot-machine roll, wrapped in `motion.div` listStagger, each card `listItem` for 60ms fade-and-rise. Chart panel reveals via `clip-path inset(0 100% 0 0)` → `inset(0 0 0 0)` whileInView. Skeleton scaffold updated to mirror the new layout.
- **β2** `worktree-agent-ae7ea24fb2caf7cda` (commit `b289565`) — Login "Gym Door" split-screen. Left half: full-bleed B&W barbell macro (Logan Weaver — `photo-1583454110551-21f2fa2afe61`) with `animate-ken-burns` (12s, scale 1→1.04), top-left ember bloom, bottom-right linear fade to `#0A0A0B`. Right half: surface-shell with faint ember radial. Logomark renders via `motion.svg` with each `<rect>` fade+scale staggered 180ms apart on first visit per tab (sessionStorage `login.logomark.played.v1`). Lowercase verb copy throughout — header `open the gym`, button `open the gym`/`opening`, labels `email`/`password`. Returning operator greeting `welcome back to {gym_name}` in `text-brand-400/60`. PrimaryButton with `magnetic={true}`. All existing auth flows preserved.
- **β3** `worktree-agent-ad4d00fc584c04b6d` (commit `1a0d7b7`) — Grow "Poster Forge". Concrete-wall background (`photo-1617791160505-6f00504e3519`) washed with `rgba(0,0,0,0.45)` + `backdrop-filter: brightness(0.55)` so it reads as backdrop, not focal. Centered on the wall: a CSS-rendered `aspect-[210/297]` paper card (420px A4 / 520px A3) with paper-grain gradient, accent header band sourced from `gym.accent_color`, gym name, live QR served from `api.qrserver.com` keyed off invite code, invite code in JetBrains Mono. Hover on A4/A3 PrimaryButtons interpolates `rotateY` (-2°/-6° vs -4° baseline) on top of `perspective(1200px) rotateX(8°)` — operator examining the paper. Click Download fires the existing PDF blob chain AND triggers a parabolic-arc exit via AnimatePresence (`y: [0,-200,-400], x: [0,100,200], scale: [1,0.9,0.4], opacity: [1,0.8,0], rotateZ: [0,8,18]`, 0.8s easeOut), then bumps a React key to snap back. `useReducedMotion` collapses the arc to a 0.18s fade.
- **β4** `worktree-agent-a215af650c6c84b15` (commit `179d0ff`) — Members "Roster" + Subscription "Receipt". Members defaults to a responsive 4/6/8/10-column bento grid of mono-avatar tiles, each with a 6-bar deterministic attendance sparkline + an ember PR ring on `workout_count > 5`. Hover inflates 1.4× via spring (380/22) while siblings dim to 30%; small overlay surfaces email + last-lift. `Roster | Table` toggle (top-right, persisted to `localStorage('members_view')`) flips to the lifted-styling table view; both share fetch/search/filter/pagination state. Empty state: outline-barbell SVG + coach copy linking to `/grow`. Subscription rendered as a printed gym receipt — `bg-ink-50` paper card with radial-gradient perforated edges, mono numerals with dotted-leader line items, rotated semi-opaque ember `PAID` stamp on `status === 'active'`. Triangular "tear-off corner" handles cancellation. Below: upgrade tier picker survives intact; the recommended Growth tier gets a 12s conic-gradient foil rotation (frozen under reduced motion).

**Merge sequence (8 `--no-ff` merges, all conflict-free thanks to file-ownership partitioning):**
α3 → α4 → α1 → α2 → β1 → β2 → β3 → β4. Total 8 merge commits + 8 worktree commits.

**Worktree gotcha hit (3 of 8 agents):** absolute paths to `C:\Users\...\admin\src\...` resolved to the main checkout, not the worktree. Alpha-3, Beta-2, and Beta-3 all caught it themselves and self-corrected by checking out the main tree clean and re-applying edits in the worktree path. Worth documenting for next sprint: agent prompts should emphasize `process.cwd()`-relative paths over absolute ones.

**Verified:** `npm run -w admin build` clean. Bundle 778 → 951 kB pre-gzip (+173) / 227 → 283 kB gzipped (+56 kB). Within the council's +60-80 kB gz target. CSS 21 → 37 kB / 5 → 7 kB gz for all the new utility classes.

**Phase work-track status:** the design overhaul is now ahead of where the original 4-PR plan would've landed. Remaining polish (modal-from-click-point, page-title shared layoutId for cross-route morph, scroll-driven reveals on Members table, anti-pattern audit on emoji/green-checkmarks) is out of scope for this sprint and tracked separately.

**Deploy notes:** no migration. Push triggers Railway (no backend change beyond the deps install) + Vercel admin redeploy. First load may be slower due to the +400 kB of Unsplash imagery; subsequent navigations are warm. The Live Pulse Strip is timer-faked for v1 — wiring it to real workout events is a Phase E item.
