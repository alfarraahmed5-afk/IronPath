...
export default function FollowingScreen() {
  ...
  const [items, setItems] = useState<FollowingRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get<{ data: { following: FollowingRow[] } }>(`/users/${user.id}/following?limit=50`);
        setItems(res.data.following ?? []);
      } catch {
        // silent
      } finally {
        setLoading(false);
  ...
  return (
    <View className="flex-1 bg-gray-950">
      <View className="flex-row items-center px-4 pt-14 pb-4 border-b border-gray-800">
        ...
        <Text className="text-white font-bold text-xl">Following</Text>
      </View>
