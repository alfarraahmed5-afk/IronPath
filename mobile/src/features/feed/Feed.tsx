/**
 * Feed feature -- absorbed from legacy (tabs)/index.tsx during the
 * Option B IA restructure. Surfaces the same workout card + comment-modal
 * UX, but now lives inside the Community tab's Feed sub-tab.
 *
 * Caller wires up the bell (notifications) -- the feature itself just
 * renders the filter pills, list, comment modal, and like/comment
 * actions.
 */
...
import { Heart, MessageCircle, Clock, Weight, Hash } from 'lucide-react-native';
...
function CommentModal({
  ...
  }, [visible, workoutId, loadComments]);
      ...
      <SafeAreaView style={styles.modalRoot}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={<EmptyState illustration="comments" title="Be the first to comment" />}
            ListFooterComponent={
              nextCursor ? (
                <TouchableOpacity onPress={() => loadComments(nextCursor!)} style={styles.loadMore}>
                  {loadingMore ? <ActivityIndicator color={colors.brand} /> : <Text variant="label" color="brand">Load more</Text>}
                </TouchableOpacity>
              ) : null
            }
          />
  ...
  return (
    ...
    >
      <Surface level={2} style={styles.card}>
        ...
        >
          <Avatar username={workout.user.username} avatarUrl={workout.user.avatar_url} size={40} />
        ...
        <View style={styles.actions}>
          <TouchableOpacity
            onPress={(e) => {
              ...
              if (wasLiked) haptic.likeOff(); else haptic.likeOn();
...
export interface FeedProps {
  /** Optional header rendered inside the list (sticks above the first card). */
  ListHeaderComponent?: React.ReactElement | null;
  /** Hide the filter pills when true (e.g. when Home renders a 3-card preview). */
  hideFilterPills?: boolean;
  /** Initial filter -- defaults to 'all'. */
  initialFilter?: FeedFilter;
  /** Hard limit -- when set, show only N rows (Home preview). */
  limit?: number;
}
...
export default function Feed({
  ...
  const fetchFeed = useCallback(
    async (opts: { filter: FeedFilter; cursor?: string; isRefresh?: boolean }) => {
      ...
      try {
        ...
        setError("Couldn't load feed. Pull down to retry.");
      } finally {
        setLoading(false);
        setRefreshing(false);
        setLoadingMore(false);
  ...
  const displayed = limit ? workouts.slice(0, limit) : workouts;
    ...
    <View style={{ flex: 1 }}>
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          onEndReached={() => {
            if (!limit && nextCursor && !loadingMoreRef.current) {
              fetchFeed({ filter, cursor: nextCursor });
            }
          }}
          onEndReachedThreshold={0.4}
          initialNumToRender={limit ?? 8}
            ...
            !loading ? (
              <View style={styles.emptyWrap}>
                <EmptyState
                  ...
                  title={filter === 'following' ? 'Follow members to see their lifts' : 'Nothing logged today. Be the first.'}
                  description={
                    filter === 'following'
                      ? "Find your gym crew. Follow a few people, your feed lights up."
                      : "When members complete workouts, you'll see them here."
                  }
                />
              </View>
            ) : null
